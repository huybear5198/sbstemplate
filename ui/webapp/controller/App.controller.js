sap.ui.define([
	...
	return MyController.extend("com.bosch.sbs.vuu7hc.ui.controller.App", {
		...
		onAfterRendering: function () {
			...
			this.storeSubscribe("showSessionTimeOut", function(cName, tName, context){
				if (context.payload) {
					if (!this._oDialog) {
						Fragment.load({
							name: "com.bosch.sbs.vuu7hc.ui.fragment.SessionTimeOutDialog",
							controller: this
		...
		checkAuthorizationValid: function () {
			...
			if(flag){
				...
				// 		name: "com.bosch.sbs.vuu7hc.ui.fragment.NotAuthorized",
				// 		controller: this
				// 	}).then(oDialog => {
		...
		doLogout: function () {
			...
			window.location.replace("/comboschsbsvuu7hcui/do/logout");
